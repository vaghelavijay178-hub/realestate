...
//
//  HOW TO GET YOUR YOUTUBE ID:
//  1. Upload your video to YouTube (set visibility to "Unlisted")
//  2. Open the video — URL looks like:
//     https://www.youtube.com/watch?v=abc123xyz
//                                    ^^^^^^^^^ this is your ID
//  3. Copy only that part and paste below (keep the quotes)
//
//  TITLE & LOCATION: change the text to match your videos
// ============================================================
const REELS = [
  {
    id: "YOUR_YOUTUBE_ID_1",     // ← paste YouTube ID e.g. "abc123xyz"
    title: "Sapno Ka Aangan",    // ← your video title
    location: "Makarpura, Gujarat",
    thumb: "",
  },
  {
    id: "YOUR_YOUTUBE_ID_2",     // ← paste YouTube ID
    title: "Your Second Video",
    location: "Your Location",
    thumb: "",
  },
  {
    id: "YOUR_YOUTUBE_ID_3",     // ← paste YouTube ID
...
export default function Home() {
  ...
  // carousel page (0 = first 3 videos, 1 = next 3, etc.)
  const [page, setPage] = useState(0);
  const PER_PAGE = 3;
  const totalPages = Math.ceil(REELS.length / PER_PAGE);
    ...
    <>
      <style>{`
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .hero-bg { position: absolute; inset: 0; background: radial-gradient(ellipse 80% 60% at 50% 80%, rgba(244,80,10,0.55) 0%, rgba(120,30,5,0.3) 45%, transparent 75%), linear-gradient(180deg, #0A0A0A 0%, #0d0d0d 100%); z-index: 0; }
        .hero-bg::after { content: ''; position: absolute; inset: 0; background: radial-gradient(ellipse at 50% 100%, rgba(244,80,10,0.18) 0%, transparent 55%); }
        ...
        .pkg-features li { display: flex; align-items: flex-start; gap: 10px; font-size: 0.8rem; color: rgba(255,255,255,0.6); padding: 8px 0; border-bottom: 1px solid rgba(255,255,255,0.06); line-height: 1.4; } .pkg-features li a { color: inherit; text-decoration: underline; text-decoration-color: rgba(255,255,255,0.2); text-underline-offset: 3px; transition: color 0.2s, text-decoration-color 0.2s; } .pkg-features li a:hover { color: var(--white); text-decoration-color: var(--orange); } .package-card.featured .pkg-features li a:hover { color: var(--white); text-decoration-color: rgba(255,255,255,0.8); }
        ...
        /* ═══════════════════════════════════════
           REELS SECTION — CAROUSEL
        ═══════════════════════════════════════ */
        .work-section { padding: 80px 48px 100px; background: var(--black); }
        .work-header { margin-bottom: 48px; display: flex; justify-content: space-between; align-items: flex-end; }
        ...
        .carousel-btn { width: 44px; height: 44px; border-radius: 50%; border: 1.5px solid var(--gray); background: transparent; color: var(--white); display: flex; align-items: center; justify-content: center; cursor: pointer; transition: border-color 0.2s, background 0.2s; flex-shrink: 0; }
        .carousel-btn:hover { border-color: var(--orange); background: var(--orange); }
        .carousel-btn:disabled { opacity: 0.25; cursor: default; }
        .carousel-btn:disabled:hover { border-color: var(--gray); background: transparent; }
        .carousel-counter { font-family: 'Space Mono', monospace; font-size: 0.65rem; color: var(--text-muted); letter-spacing: 0.1em; margin-right: 4px; }
        .reels-carousel { overflow: hidden; }
        .reels-track { display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px; transition: transform 0.5s cubic-bezier(0.25,0.46,0.45,0.94); }
        .reel-card { position: relative; border-radius: 20px; overflow: hidden; background: #1a1a1a; cursor: pointer; border: none; outline: none; aspect-ratio: 3 / 4; transition: transform 0.3s ease; min-width: 0; }
        ...
        @media (max-width: 768px) {
          ...
          .hero { padding: 0 20px 48px; min-height: 100svh; } .hero-property-tag { display: none; } .hero-tagline { display: none; } .hero-title { font-size: clamp(3.5rem, 15vw, 6rem); } .hero-services { gap: 20px; flex-wrap: wrap; }
        ...
        <div className="packages-grid">
          <div className="package-card reveal">
            ...
            <ul className="pkg-features">
              <li><a href="https://www.instagram.com/_k.elsie/?hl=en">P2C property introduction reel</a></li>
              <li><a href="https://www.instagram.com/reels/DV_L5UcDFYj/">VO amenities highlight reel</a></li>
              <li><a href="https://www.instagram.com/reel/DWRKqskoVyG/?hl=en">Scroll-stopping eye-catching reel</a></li>
            </ul>
          ...
          <div className="package-card featured reveal reveal-d1">
            ...
            <ul className="pkg-features">
              ...
              <li><a href="https://www.instagram.com/reel/DVJA78aDdfH/?hl=en">Location benefits reel</a></li>
              <li><a href="#work">Carousel post + 2 Instagram story images</a></li>
            </ul>
          ...
          <div className="package-card reveal reveal-d2">
            ...
            <ul className="pkg-features">
              ...
              <li><a href="#work">Cinematic property walk-through film</a></li>
            </ul>
      ...
      {/* ══════════════════════════════════════════
          REELS SECTION — CAROUSEL (3 per slide)
      ══════════════════════════════════════════ */}
      <section className="work-section" id="work">
        <div className="work-header reveal">
          ...
          <div className="carousel-arrows">
            <span className="carousel-counter">
              {Math.floor(playing !== null ? 0 : 0) + 1}
            </span>
            <button
              className="carousel-btn"
              onClick={() => { setPlaying(null); setPage((p) => Math.max(0, p - 1)); }}
              disabled={page === 0}
              aria-label="Previous"
            >
            ...
            <button
              className="carousel-btn"
              onClick={() => { setPlaying(null); setPage((p) => Math.min(totalPages - 1, p + 1)); }}
              disabled={page === totalPages - 1}
              aria-label="Next"
            >
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M6 3L11 8L6 13" stroke="white" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg>
            </button>
          </div>
        </div>
        ...
        <div className="reels-carousel">
          <div className="reels-track">
            {REELS.slice(page * PER_PAGE, page * PER_PAGE + PER_PAGE).map((reel, i) => {
              const globalIndex = page * PER_PAGE + i;
              return (
              <div
                key={i}
                className="reel-card"
                onClick={() => playing !== globalIndex && setPlaying(globalIndex)}
                role="button"
                tabIndex={0}
                onKeyDown={(e) => e.key === "Enter" && setPlaying(globalIndex)}
              >
                {playing === globalIndex ? (
                  <>
        ...
        <div style={{display:"flex", justifyContent:"center", gap:"8px", marginTop:"28px"}}>
          {Array.from({length: totalPages}).map((_, i) => (
            <button
              key={i}
              onClick={() => { setPlaying(null); setPage(i); }}
              style={{
                width: i === page ? "24px" : "8px",
                height: "8px",
                borderRadius: "100px",
                background: i === page ? "var(--orange)" : "var(--gray)",
                border: "none",
                cursor: "pointer",
